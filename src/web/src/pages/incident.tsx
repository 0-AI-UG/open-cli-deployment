...
export function IncidentPage({ id }: { id: string }) {
  ...
  return <PageShell width="md">
    <PageHeader title="Incident response" eyebrow="Outage" description={data?.incident.title || "Loading incident…"} backHref={data ? `#${data.incident.path}` : "#/"} backLabel="Back to resource" actions={<Btn onClick={() => void load()}><RefreshCw size={13} /> Refresh</Btn>} />
