...
export function IncidentPage({ id }: { id: string }) {
  ...
  return <PageShell>
    <PageHeader title="Incident details" eyebrow="Operations" description={incident?.title || "Outage and recovery history"} backHref="#/incidents" backLabel="Back to incidents" actions={<Btn disabled={loading} onClick={() => void load()}><RefreshCw size={13} /> Refresh</Btn>} />
    ...
    {incident && guide && <>
      <Card className="space-y-4 p-5">
        ...
        <p className="text-sm leading-relaxed">{resolved ? "The monitored condition has cleared. This record is retained for review." : guide.condition}</p>
      ...
      <Card className="space-y-4 p-5">
        <h2 className="font-mono text-xs font-bold uppercase">{resolved ? "Review and prevent recurrence" : "Investigation steps"}</h2>
        <ol className="list-decimal space-y-3 pl-5 text-sm leading-relaxed">{guide.steps.map(step => <li key={step}>{step}</li>)}</ol>
        <p className="border-t border-fg/15 pt-4 text-xs leading-relaxed text-muted">{guide.recovery} Active incidents refresh every 15 seconds. Status is updated automatically by monitoring.</p>
      </Card>
      <Card className="space-y-4 p-5">
        ...
        <ol className="space-y-4 border-l-2 border-fg/20 pl-4 text-xs">
          ...
          <li><div className="font-bold">{resolved ? "Condition cleared" : "Awaiting recovery"}</div><span className="text-muted">{resolved ? incidentDate(incident.resolved_at) : "The incident remains open until monitoring detects that the condition has cleared."}</span></li>
        </ol>
