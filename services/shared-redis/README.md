...
The password is stored as `SHARED_REDIS_PASSWORD` in `shared-data`. Each consuming project stores its own secret `REDIS_URL` in its application environment, pointing at `sight-redis.ocd.internal:6379`. Use project-specific key prefixes to avoid collisions. Redis is used for caches and short-lived coordination; durable application state belongs in PostgreSQL.
